...
export default function MisCitasPage() {
  ...
  const submitRating = async () => {
    ...
    try {
      ...
      if (ratingId) {
        const url = `http://http:localhost:3000/calificaciones/medicos/${ratingId}`;
        ...
        const resp = await fetch('http://http:localhost:3000/calificaciones/medicos', {
          method: 'POST',
          credentials: 'include',
          headers: { 'Content-Type': 'application/json' },
  ...
  const loadExistingRating = async (medicoId?: string | null, citaId?: string | null) => {
    ...
    if (!medicoId) return;
    try {
      const resp = await fetch('http://http:localhost:3000/calificaciones/mis-calificaciones?tipo=medicos', {
        method: 'GET',
        credentials: 'include',
      });
      if (!resp.ok) {
