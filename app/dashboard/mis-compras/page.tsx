...
export default function MisComprasPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const resp = await fetch('http://localhost:3000/productos/mis-compras?limit=20&offset=0', { credentials: 'include', signal: ctrl.signal });
        if (!resp.ok) {
        ...
        let ratings: any[] = [];
        try {
          const r = await fetch('http://localhost:3000/calificaciones/mis-calificaciones?tipo=productos', { credentials: 'include' });
          if (r.ok) {
  ...
  const loadExistingProductRating = async (productoId: string, compraId: string) => {
    try {
      const resp = await fetch('http://localhost:3000/calificaciones/mis-calificaciones?tipo=productos', { credentials: 'include' });
  ...
  const submitRating = async () => {
    ...
    try {
      ...
      if (ratingId) {
        ...
        const resp = await fetch(`http://localhost:3000/calificaciones/productos/${ratingId}`, {
          method: 'PUT',
          credentials: 'include',
        ...
        const resp = await fetch('http://localhost:3000/calificaciones/productos', {
          method: 'POST',
          credentials: 'include',
          headers: { 'Content-Type': 'application/json' },
